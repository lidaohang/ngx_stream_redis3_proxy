...
inline std::string StringUtil::ToString(int _Val)
{   // convert long long to string
    char _Buf[2 * 64];
    int len;

    len = snprintf(_Buf,sizeof(_Buf), "%d", _Val);
    return (std::string(_Buf,len));
}

inline std::string StringUtil::DoubleToString(double _Val)
{   // convert long long to string
    char _Buf[2 * 64];
    int len;

    len = snprintf(_Buf,sizeof(_Buf), "%lf", _Val);
    return (std::string(_Buf,len));
}
